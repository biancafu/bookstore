...
exports.handler = async (event) => {
  try {
    let payload = event.prev.result; //get results passed down from the previous function (the processPayment function)
    payload.order_id = uuidv4();

    // create a new order using aws dynamo db sdk
    await createOrder(payload); 

    // links books with the order (many to many relationship between orders and books handled by type bookorder)
